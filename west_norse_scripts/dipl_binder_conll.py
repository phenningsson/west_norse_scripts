...
def convert_to_conll(
    ...
    unique_in_json = len(set((e['dipl_text'], e['label']) for e in entities))
    ...
    not_found = []
    for ent in entities:
        if ent['dipl_text'].lower() not in found_entity_texts:
            key = (ent['dipl_text'], ent['label'])
            if key not in [(n[0], n[1]) for n in not_found]:
                not_found.append((ent['dipl_text'], ent['label']))
...
def main():
    ...
    # File paths 
    text_path = '/Users/phenningsson/Downloads/west_norse_scripts/flat_ner_diplomatic/menota/wormianus/wormianus_final_text.txt'
    entities_path = '/Users/phenningsson/Downloads/west_norse_scripts/flat_ner_diplomatic/menota/wormianus/dipl_wormianus_entities.json'
    output_path = '/Users/phenningsson/Downloads/west_norse_scripts/menota_conll_diplomatic/diplomatic_conll_wormianus.txt'
    ...
    for ent in entities[:10]:
        print(f"  {ent['dipl_text']:20s} -> {ent['label']}")
